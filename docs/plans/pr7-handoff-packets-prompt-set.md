...
§4 PR 7, §6 "Per-item BLESS", §7 row 7. Repos: `crossr-loops`, `crossr-skills`.
...
| crossr-harness | `1d60943` | `v1-one-law` | `v1-one-law-consumers` | — | not in this stack |
...
## Stack (three PRs, merge order is stack order)
...
7b  loops    skills pin → v1-packets; graph node `batch`; verify-protocol batch rule;
             avril batch review + per-item tokens; PO/QA/CTO personas; axel packet
             ritual + references/handoff-packet.md; test/test_pr7.py
             → cut loops tag v1-packets-consumers on the merge commit
7c  skills   loops pin → v1-packets-consumers; just regen-agents ×2; plan record
...
Numbered so a brief can cite them. Each is a default the human may overturn; none is open.
...
1. **A packet is a file, or it is not audited.** §3.8 says "conductor sends brief + envelope
   ...
   packet to the disclosed scratch path, runs `audit-packet brief` on it, and only a green
   packet is delegated. Packets are ephemeral: never committed, never a tracker. The 20-line
   Completion Record is the memory (§3.8 last paragraph).
...
9. **No batch-size cap.** The plan sets none. The per-item token is the guard. Recorded here
   so a later "add a cap" is a decision, not drift.
10. **Card byte caps are deltas, measured.** `avril/SKILL.md` ≤ 5,400 B (4,984 + ≤ 416),
    ...
    is frozen (decision #12): 7c appends a **new** row-7 line and does not touch row 4.
    Everything that is a template or an explanation goes in `references/`.
11. **Harness is a follow-up card, not this stack.** The issue names loops and skills.
    Harness needs: pin bump to the two new tags, `audit-packet` installed by bootstrap next
    to `audit-plan` (HARNESS-SPEC §6 pattern), smoke updated. That is a `work.json` hygiene
    item blocked by `pr7`; 7c's ledger step adds it. Do not fold harness edits into 7b/7c.
...
- Read docs/plans/gan-layer-separation-plan.md (§3.3, §3.8, §4 PR 7, §6, §7 row 7 govern
  this work), docs/plans/pr7-handoff-packets-prompt-set.md (decisions 1–12), and AGENTS.md
...
- Enumerate EVERY protected-law edit in the PR body, one numbered item each, with the
  ...
  schema). Undisclosed law edits are an automatic REJECT (#107 precedent).
...
1. gan-verdict/SKILL.md — append, do not rewrite. Existing items 1–7 keep their numbers
   ...
   8. Handoff packet — inbound field schema (conductor fills; adversary receives nothing
      ...
      HTML. Grammar: `references/handoff-packet.md`. Gate: `audit-packet brief`.
...
6. test/test_pr7.py — the test_pr6.py pattern (module docstring names the brief and the
   ...
   tree). Required assertions, each its own test:
     ...
     - LiveTree: gan-verdict/SKILL.md ≤ 2,048 B; contains the decision-3 sentence
       ...
       audit-packet brief when extracted from its fence; README table == public-skills
       .json still holds (docs-verify runs in harness-validate; do not duplicate it here).
...
Stack line for the body: "7a of 3. Nothing merges before. Cut skills tag v1-packets on
...
REJECT loops the item, not the siblings; AXEL delegates only audited packets and drops
review prose after commit. Decisions 3, 4, 5, 8, 9, 10.
...
5. references/batch-review.md: the delegation shape (persona + gate/avril card + the
   ...
   material-edit rule, and decision 9 (no size cap, recorded).

6. PO / QA / CTO personas. Each already ends "every reviewed item" with
   ...
   `code-writer` in their Required Skills — out of scope, note it as a park if you
   think it is wrong.

7. avril-conductor-agent.md steps 5–6: delegate the set, run the verdict gate, loop the
   rejected id only. Keep the One-Sentence Mandate byte-identical (paste grep).

8. axel/SKILL.md §4 (delta ≤ 407 B). Before each adversary delegation: write the
   handoff packet (references/handoff-packet.md) to the disclosed scratch path, run
   `audit-packet brief` (mechanical, zero tokens), red → fix the packet, green →
...
9. axel/references/handoff-packet.md: the AXEL fill-in of gan-verdict's grammar —
   ...
   from item 5). Name the scratch path convention as a harness-disclosed parameter
   (harness-parameters.md owns paths; add one line there naming it "packet scratch
   path; never committed"). Link, do not copy, the gan-verdict grammar.

10. completion-record.md: one line under `## Phases`: "- retained: this record; review
    prose dropped (§3.8)".

11. axel-conductor-agent.md step 8: insert the packet audit before Tester and Reviewer
    and the verdict audit after each. Mandate byte-identical (paste grep).

12. Book pages and the /avril command: one paragraph in avril.md (set review, per-item
    tokens, sibling BLESS survives), one in axel.md (packet in, audited; prose out after
    commit); avril.md command template `review <ids>` route gains "(one verdict line
    per id)". No other template edit.

13. test/test_pr7.py in the test_pr6.py shape. Required:
    ...
    LiveTree:
      ...
      - avril card contains the decision-3 sentence verbatim, "one verdict line per id"
        (case-insensitive), "audit-packet verdict", and "siblings"
...
14. features.json: gan-layer-separation `pr7b` row, status completed, features
    ["batch-topology", "verify-protocol-batch", "avril-set-review",
...
VALIDATE (paste all, in order):
  ...
  grep -n 'One-Sentence Mandate' -A1 .agents/agents/{avril,axel}-conductor-agent.md     → unchanged vs main (paste diff = empty)
...
Stack line: "7b of 3. 7a merged as <sha>, skills tag v1-packets → <sha>. Cut loops tag
v1-packets-consumers on the merge commit after re-running this block on it. 7c (skills
closer) is next."
```

---

## Brief 7c — skills: pin bump, regen, plan record, phase close

```
You are implementing crossr v2 PR 7c in sycamore-hq/crossr-skills, on a branch off main
at or after the 7a merge. Work item: sycamore-hq/work#12.
Precondition: loops tag v1-packets-consumers exists; paste the peeled SHA. If not, stop.

[paste GUARDRAILS]

GOAL: the catalog consumes the loops it just made possible, and the plan record stops
lying about PR 6. Decisions 10, 11.
...
FILES (exhaustive):
  ...
  test/test_pr5f.py                                    PIN constant → v1-packets-consumers (the test that asserts four loci agree)
  test/test_pr7.py                                     + record assertions
  features.json, progress.md                           pr7c row; phase status
...
2. Plan record, both twins, hand-maintained, must agree:
   ...
   - `### PR 7 — …` → `✅ landed` with links to the three PRs and both tags.
   - Header `**Status:**` line: append `· PR 6 landed (…) · PR 7 landed (…; tags
     v1-packets / v1-packets-consumers)` and change the leading word to `complete`
     when you close the phase (item 4 — test_gan_record.phase_disagrees_with_plan
     fails a completed phase under an "in progress" header).
   - §7 row 7 is a draft until this merge (decision #12), so edit it in place to the
     ...
     three batch adversaries." Fill N from the 7a suite. Do not touch rows 0–6.
   - §6 "Per-item BLESS when AVRIL batches (PR 7)" → keep; append "(landed: gan-verdict
     item 9 + verify-protocol batch rule)".
   - §8: add decision 13, one row, pointing at this prompt set's decisions 1–12 as the
     PR 7 settlement (the PR 5 decisions got the same treatment).

3. Prompt set Status table: add a `## Status (measured <date>)` section to
   pr7-handoff-packets-prompt-set.md in the pr5 shape — 7a / 7b / 7c rows with PR
   links, merge SHAs, tags. No HTML twin exists for this file (see Unresolved 1).

4. Tracker: features.json `pr7c` row completed; gan-layer-separation phase status →
   completed IF §4 has no unlanded PR after this one (it does not: PR 7 is last; §5 is
   "also worth doing", not units). progress.md PR 7c (COMPLETED) + a phase-close note.

5. test/test_pr7.py additions (LiveTree): plan header contains "PR 6 landed" and "PR 7
   landed"; both `### PR 6` and `### PR 7` headings carry ✅; html twin names both tags;
   phase status agrees with the header (reuse test_gan_record's calculation); all four
   pin loci read v1-packets-consumers (test_pr5f already asserts this once PIN moves —
   do not duplicate, just move PIN).
...
Stack line: "7c of 3, closes PR 7. 7b merged as <sha>, loops tag v1-packets-consumers →
<sha>. No tag cut here. Next: work ledger flip + harness follow-up card."
```
...
`work.json` is the record; issues and the Project are views. Two ledger commits, both
with `just test` + `just status-html` pasted:

1. **At 7a merge** — `pr7` `todo` → `in_progress`; `notes` names skills#<7a> and the
   `v1-packets` SHA; `source_as_of` updated. Commit message in the house shape
   (`work-00: pr7 in progress on skills#<n> <range>`).
2. **At 7c merge + both tags** — `pr7` → `done`; `notes` carries the three PR ranges and
   both tags. Add hygiene item `harness-pins-pr7` (`todo`, repos `crossr-harness`,
   `blocked_by: ["pr7"]`, source `crossr-harness/lockfile.toml vs tags v1-packets /
   v1-packets-consumers; HARNESS-SPEC §6 audit-packet install; smoke pins`) — decision 11.
   Startable count on the board becomes that item alone.
3. `just project-roadmap --dry-run` pasted; the write needs org project write (a human
   runs `just project-roadmap`). `progress.md` gets the pr7 done paragraph in the pr6 shape.

The ledger tests assert invariants, not ids; a new item needs an `issue` number — open the
issue first (`just project-roadmap` will label it), then add the row.
...
- **7a**: gan-verdict items 1–7 are context lines in the diff; ≤ 2,048 B measured; the
  ...
  no rust/ocaml token; harness-validate pasted; features/progress appended, not rewritten;
  stack line present; tag cut after re-validation on the merge commit.
- **7b**: pin moved at both loci; verify-skill-refs pasted **red at v1-one-law and green at
  ...
  three batch personas; the three AXEL verdict-format lines and both conductor mandates
  byte-identical (diff empty); card caps measured; axel card names both audit modes and
  the prose-drop; no gan-verdict grammar copied into loops (linked by name); book and
  command edits are one paragraph / one hint each; index.html regenerated twice clean;
  tag cut after re-validation.
- **7c**: regen twice clean and the persona diff is exactly 7b's; PR 6 recorded landed
  (it never was); header says complete iff the phase is completed; §7 row 7 filled with
  the 7a test count, rows 0–6 untouched; decision 13 row present; prompt-set Status table
  added; test_pr5f PIN moved, not duplicated; harness-validate green.
- **Throughout**: every acceptance line has pasted output; a "verified" without a command
  is a REJECT; no new verdict token; no book name; nothing outside the file lists.
...
2. **`code-writer` on the AVRIL personas.** PO / QA / CTO still require `code-writer`; the
   avril card says "load no writer skill". Out of PR 7's scope; 7b parks it. Needs a card.
3. **Scratch path for packets.** Default is a harness-disclosed parameter (7b item 9). If
   the human wants a fixed convention (`docs/plans/pbi/<id>.phase-<k>.<gate>.packet.md`,
   gitignored), say so before 7b dispatches; otherwise it stays disclosed.
4. **Batch size cap** — decision 9 records "none". Overturn before 7b if a cap is wanted.
5. **Phase close in 7c.** Default closes `gan-layer-separation` on skills since PR 7 is the
   last §4 unit. If §5 "also worth doing" items are meant to become units of this phase,
   keep it open and say so; the header then stays "in progress".
6. **Harness follow-up** is a new ledger card (decision 11), not a fourth brief. Confirm.
