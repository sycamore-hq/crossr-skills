...
- The agent applies combinators → match/guards → helper extraction as a fail-closed order: no `match` / `function` / `if` inside any arm of a `match` or `function`, no `match` under `then` / `else`, no identity `| Error e -> Error e`, and `let*` meaning `Result.bind` / `let**` meaning `Option.bind` in every file.
