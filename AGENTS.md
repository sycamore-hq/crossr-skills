...
- `lockfile.toml` — Consumer pins: `skills = "v1-model-ask"`, `loops = "v1-model-ask-consumers"`. Pins, not work state. Graphs live in `crossr-loops/graphs/` and are in the `v1-model-ask-consumers` pin. Topology only — if a graph and a conductor `SKILL.md` disagree, the skill wins.
