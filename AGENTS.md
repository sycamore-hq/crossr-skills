...
- `lockfile.toml` — Consumer pins: `skills = "v1-gan-layers"`, `loops = "v1-board-consumers"`. Pins, not work state. Graphs live in `crossr-loops/graphs/` and are in the `v1-board-consumers` pin. Topology only — if a graph and a conductor `SKILL.md` disagree, the skill wins.
